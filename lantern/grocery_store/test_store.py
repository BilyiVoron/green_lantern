...
class Initializer:
    ...
    @staticmethod
    def setup_store():
        return {"name": "Mad Cow", "location": "Lviv", "manager_id": 2}
...
class TestUsers(Initializer):
    ...
    def test_get_nonexistent_user(self):
        resp = self.client.get(f"{self.user_route}/1")
    ...
    @pytest.mark.parametrize("current_user_id,expected_user_id", ((1, 5),))
    def test_update_nonexistent_user(self, current_user_id, expected_user_id):
        self.client.post(self.user_route, json=self.user)
        resp = self.client.get(f"{self.user_route}/{expected_user_id}")
...
class TestStores(Initializer):
    ...
    @pytest.mark.parametrize("current_store_id,expected_store_id", ((1, 5),))
    def test_update_nonexistent_store(self, current_store_id, expected_store_id):
        self.client.post(self.store_route, json=self.stores)
        resp = self.client.get(f"{self.store_route}/{expected_store_id}")
